...
void Data_Flash_read_indirect(void)
{
    unsigned long check_read_data_addr = 0;
    pfdl_status_t status ;
    
    check_read_data_addr = WRITE_ADDRESS;

    DI(); 
    status = (pfdl_status_t) data_flash_read_packet_data_indirect(check_read_data_addr,data_flash_data_read,WRITE_DATA_LENGTH);
    printf_tiny("Data_Flash_read_indirect finish:r(0x%02X)\r\n",status);
    EI();

    dump_buffer_hex(data_flash_data_read ,WRITE_DATA_LENGTH);
}

void Data_Flash_read_direct(void)
{
    unsigned long check_read_data_addr = 0;
    pfdl_status_t status ;
    ...
    DI(); 
    status = (pfdl_status_t) data_flash_read_packet_data_direct(check_read_data_addr,data_flash_data_read,WRITE_DATA_LENGTH);
    printf_tiny("Data_Flash_read_direct finish:r(0x%02X)\r\n",status);
    EI();
...
void Data_Flash_write_test(void)
{
    ...
    status = (pfdl_status_t) data_flash_erase_app_area(WRITE_ADDRESS);
    printf_tiny("Data_Flash_write_test finish:e(0x%02X)\r\n",status);
    status = (pfdl_status_t) data_flash_write_packet_data(WRITE_ADDRESS,data_flash_data_write,WRITE_DATA_LENGTH);
    printf_tiny("Data_Flash_write_test finish:w(0x%02X)\r\n",status);
...
void Data_Flash_init(void)
{
    pfdl_status_t status ;
    DI();    
    status = (pfdl_status_t) init_fdl();
    printf_tiny("Data_Flash_init finish:i(0x%02X)\r\n",status);
    EI();
}
...
void hardware_init(void)
{
    ...
    Data_Flash_init();
    Data_Flash_read_indirect();//Data_Flash_read_direct();
    Data_Flash_write_test();
    Data_Flash_read_indirect();//Data_Flash_read_direct();
